...
static PyObject* py_solvePoisson1D(PyObject* self, PyObject* args)
{
  ...
  // I set the number of total points in the 1D domain
  Np=d.nx;
  ...
  // I read the dielectric constant along the 1D domain
  ...
  // I read the boundary conditions along the 1D domain
...
/* 	printf("x|carica %d  %lg\n",i,p.free_charge[i]); */
  ...
  // ************************************************************************
  // Here it starts the solution of the 1D Poisson equation
  // ************************************************************************
  p.vt=dvector(0,0);
  (*p.vt)=300*kboltz/q;
  ciclomm=2;
  ...
  if (!rank) printf("*********************************\n");
  if (!rank) printf("******* Solving POISSON 1D ******\n"); 
  if (!rank) printf("*********************************\n\n");
  ...
  J=d3tensor(0,d.nx,0,neq,0,3*neq);
  IA=ivector(0,NELT);
  JA=ivector(0,NELT);
  ...
  while (normavp>normapoisson)
    {
      ...
      preparej1D(p,d,A,IA,JA,&Z1,gumgum,J,neq);
...
/*       stampaout("tipopunto",d.boundary_conditions,Np); */
...
/*       printf("ecco3\n"); */
  ...
  free_dmatrix(d.dist,0,Np-1,0,1);
  free_dvector(d.boundary_conditions,0,Np-1);
  free_d3tensor(J,0,d.nx,0,neq,0,3*neq);
