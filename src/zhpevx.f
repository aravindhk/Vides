...
*     .. External Functions ..
      ...
      EXTERNAL           LSAME1, DLAMCH, ZLANHP
*     ..
*     .. External Subroutines ..
      EXTERNAL           DCOPY, DSCAL, DSTEBZ, DSTERF, XERBLA1, ZDSCAL,
     $                   ZHPTRD, ZSTEIN, ZSTEQR, ZSWAP1, ZUPGTR, ZUPMTR
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      WANTZ = LSAME1( JOBZ, 'V' )
      ALLEIG = LSAME1( RANGE, 'A' )
      VALEIG = LSAME1( RANGE, 'V' )
      INDEIG = LSAME1( RANGE, 'I' )
*
      INFO = 0
      IF( .NOT.( WANTZ .OR. LSAME1( JOBZ, 'N' ) ) ) THEN
         INFO = -1
      ELSE IF( .NOT.( ALLEIG .OR. VALEIG .OR. INDEIG ) ) THEN
         INFO = -2
      ELSE IF( .NOT.( LSAME1( UPLO, 'L' ) .OR. LSAME1( UPLO, 'U' ) ) )
     $          THEN
         INFO = -3
      ELSE IF( N.LT.0 ) THEN
         INFO = -4
...
*
      ...
      IF ((ALLEIG .OR. TEST) .AND. (ABSTOL.LE.ZERO)) THEN
         CALL DCOPY1( N, RWORK( INDD ), 1, W, 1 )
         INDEE = INDRWK + 2*N
         IF( .NOT.WANTZ ) THEN
            CALL DCOPY1( N-1, RWORK( INDE ), 1, RWORK( INDEE ), 1 )
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           XERBLA1, ZAXPY, ZHPMV, ZHPR2, ZLARFG
*     ..
*     .. External Functions ..
      LOGICAL            LSAME1
      COMPLEX*16         ZDOTC
      EXTERNAL           LSAME1, ZDOTC
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE
*     ..
...
*
      INFO = 0
      UPPER = LSAME1( UPLO, 'U' )
      IF( .NOT.UPPER .AND. .NOT.LSAME1( UPLO, 'L' ) ) THEN
         INFO = -1
      ELSE IF( N.LT.0 ) THEN
         INFO = -2
      END IF
      IF( INFO.NE.0 ) THEN
         CALL XERBLA1( 'ZHPTRD', -INFO )
         RETURN
      END IF
*
*     Quick return if possible
...
*
      IF( N.EQ.0 ) THEN
         VALUE = ZERO
      ELSE IF( LSAME1( NORM, 'M' ) ) THEN
*
*        Find max(abs(A(i,j))).
*
         VALUE = ZERO
         IF( LSAME1( UPLO, 'U' ) ) THEN
            K = 0
            DO 20 J = 1, N
               DO 10 I = K + 1, K + J - 1
   ...
   40       CONTINUE
         END IF
      ELSE IF( ( LSAME1( NORM, 'I' ) ) .OR. ( LSAME1( NORM, 'O' ) ) .OR.
     $         ( NORM.EQ.'1' ) ) THEN
*
...
*
  ...
  100       CONTINUE
         END IF
      ELSE IF((LSAME1( NORM, 'F' ) ) .OR. ( LSAME1( NORM, 'E' ) ) ) THEN
*
*        Find normF(A).
*
         SCALE = ZERO
...
*
      INFO = 0
      IF( .NOT.( LSAME1( SIDE, 'L' ) .OR. LSAME1( SIDE, 'R' ) ) ) THEN
         INFO = 1
      ELSE IF( .NOT.( LSAME1( PIVOT, 'V' ) .OR. LSAME1( PIVOT,
     $         'T' ) .OR. LSAME1( PIVOT, 'B' ) ) ) THEN
         INFO = 2
      ELSE IF( .NOT.( LSAME1( DIRECT, 'F' ) .OR. LSAME1( DIRECT, 'B' )))
     $          THEN
         INFO = 3
      ELSE IF( M.LT.0 ) THEN
         INFO = 4
...
*
      IF( ( M.EQ.0 ) .OR. ( N.EQ.0 ) )
     $   RETURN
      IF( LSAME1( SIDE, 'L' ) ) THEN
*
*        Form  P * A
*
         IF( LSAME1( PIVOT, 'V' ) ) THEN
            IF( LSAME1( DIRECT, 'F' ) ) THEN
               DO 20 J = 1, M - 1
                  CTEMP = C( J )
                  STEMP = S( J )
...
*     .. External Functions ..
      INTEGER            IDAMAX
      DOUBLE PRECISION   DASUM, DLAMCH, DNRM21
      EXTERNAL           IDAMAX, DASUM, DLAMCH, DNRM21
*     ..
*     .. External Subroutines ..
      EXTERNAL           DCOPY1, DLAGTF, DLAGTS, DLARNV, DSCAL, XERBLA1
*     ..
*     .. Intrinsic Functions ..
...
*
            CALL DCOPY1( BLKSIZ, D( B1 ), 1, WORK( INDRV4+1 ), 1 )
            CALL DCOPY1( BLKSIZ-1, E( B1 ), 1, WORK( INDRV2+2 ), 1 )
            CALL DCOPY1( BLKSIZ-1, E( B1 ), 1, WORK( INDRV3+1 ), 1 )
*
...
*
  130       CONTINUE
            SCL = ONE / DNRM21( BLKSIZ, WORK( INDRV1+1 ), 1 )
            JMAX = IDAMAX( BLKSIZ, WORK( INDRV1+1 ), 1 )
...
*     .. External Functions ..
      ...
      EXTERNAL           LSAME1, DLAMCH, DLANST, DLAPY2
*     ..
*     .. External Subroutines ..
      EXTERNAL           DLAE2, DLAEV2, DLARTG, DLASCL, DLASRT, XERBLA1,
     $                   ZLASET, ZLASR, ZSWAP1
*     ..
*     .. Intrinsic Functions ..
...
*
      INFO = 0
*
      IF( LSAME1( COMPZ, 'N' ) ) THEN
         ICOMPZ = 0
      ELSE IF( LSAME1( COMPZ, 'V' ) ) THEN
         ICOMPZ = 1
      ELSE IF( LSAME1( COMPZ, 'I' ) ) THEN
         ICOMPZ = 2
      ELSE
         ICOMPZ = -1
      END IF
...
*
     ...
     $               LDA, WORK )
         CALL ZSCAL1( M-N+II-1, -TAU( I ), A( 1, II ), 1 )
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           XERBLA1, ZUNG2L, ZUNG2R
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     ..
...
*
      ...
      ELSE
         NQ = N
      END IF
      IF( .NOT.LEFT .AND. .NOT.LSAME1( SIDE, 'R' ) ) THEN
         INFO = -1
      ELSE IF( .NOT.UPPER .AND. .NOT.LSAME1( UPLO, 'L' ) ) THEN
         INFO = -2
      ELSE IF( .NOT.NOTRAN .AND. .NOT.LSAME1( TRANS, 'C' ) ) THEN
         INFO = -3
      ELSE IF( M.LT.0 ) THEN
         INFO = -4
      ELSE IF( N.LT.0 ) THEN
...
*
   ...
   10    CONTINUE
      ELSE IF( LSAME1( NORM, 'O' ) .OR. NORM.EQ.'1' .OR.
     $         LSAME1( NORM, 'I' ) ) THEN
*
*        Find norm1(A).
*
...
*
      INFO = 0
*
      IF( LSAME1( TYPE, 'G' ) ) THEN
         ITYPE = 0
      ELSE IF( LSAME1( TYPE, 'L' ) ) THEN
         ITYPE = 1
      ELSE IF( LSAME1( TYPE, 'U' ) ) THEN
         ITYPE = 2
      ELSE IF( LSAME1( TYPE, 'H' ) ) THEN
         ITYPE = 3
      ELSE IF( LSAME1( TYPE, 'B' ) ) THEN
         ITYPE = 4
      ELSE IF( LSAME1( TYPE, 'Q' ) ) THEN
         ITYPE = 5
      ELSE IF( LSAME1( TYPE, 'Z' ) ) THEN
         ITYPE = 6
      ELSE
         ITYPE = -1
      END IF
...
*     .. External Functions ..
      ...
      INTEGER            ILAENV1
      DOUBLE PRECISION   DLAMCH
      EXTERNAL           LSAME, ILAENV1, DLAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           DLAEBZ, XERBLA1
*     ..
*     .. Intrinsic Functions ..
...
*     Decode RANGE
*
      IF( LSAME1( RANGE, 'A' ) ) THEN
         IRANGE = 1
      ELSE IF( LSAME1( RANGE, 'V' ) ) THEN
         IRANGE = 2
      ELSE IF( LSAME1( RANGE, 'I' ) ) THEN
         IRANGE = 3
      ELSE
         IRANGE = 0
      END IF
...
*     Decode ORDER
*
      IF( LSAME1( ORDER, 'B' ) ) THEN
         IORDER = 2
      ELSE IF( LSAME1( ORDER, 'E' ) ) THEN
         IORDER = 1
      ELSE
         IORDER = 0
      END IF
...
*     ..
*     .. External Subroutines ..
      EXTERNAL           DLAE2, DLASCL, DLASRT, XERBLA1
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   60 DASUM = DTEMP
      RETURN
      END
      SUBROUTINE DCOPY1(N,DX,INCX,DY,INCY)
*     .. Scalar Arguments ..
      INTEGER INCX,INCY,N
*     ..
*     .. Array Arguments ..
...
*
   ...
   50 CONTINUE
      RETURN
      END
      DOUBLE PRECISION FUNCTION DNRM21(N,X,INCX)
*     .. Scalar Arguments ..
      INTEGER INCX,N
*     ..
*     .. Array Arguments ..
...
*  =======
*
*  DNRM21 returns the euclidean norm of a vector via the function
*  name, so that
*
*     DNRM21 := sqrt( x'*x )
*
...
*  =======
*
*  LSAME1 returns .TRUE. if CA is the same letter as CB regardless of
*  case.
*
*  Arguments
*  =========
...
*
      END
      SUBROUTINE XERBLA1(SRNAME,INFO)
*
...
*  =======
*
*  XERBLA1  is an error handler for the LAPACK routines.
...
*          The name of the routine which called XERBLA1.
*
*  INFO    (input) INTEGER
...
*
      INFO = 0
      IF (.NOT.LSAME1(UPLO,'U') .AND. .NOT.LSAME1(UPLO,'L')) THEN
          INFO = 1
      ELSE IF (N.LT.0) THEN
          INFO = 2
...
*
      END
      SUBROUTINE ZSCAL1(N,ZA,ZX,INCX)
*     .. Scalar Arguments ..
      DOUBLE COMPLEX ZA
      INTEGER INCX,N
*     ..
...
*
   ...
   30 CONTINUE
      RETURN
      END
      SUBROUTINE ZSWAP1(N,ZX,INCX,ZY,INCY)
*     .. Scalar Arguments ..
      INTEGER INCX,INCY,N
*     ..
*     .. Array Arguments ..
...
*       code for both increments equal to 1
   ...
   30 CONTINUE
      RETURN
      END
      DOUBLE PRECISION FUNCTION DCABS11(Z)
*     .. Scalar Arguments ..
      DOUBLE COMPLEX Z
*     ..
*     ..
...
*  =======
*
*  DCABS11 computes absolute value of a double complex number 
*
*     .. Intrinsic Functions ..
      INTRINSIC ABS,DBLE,DIMAG
*
      DCABS11 = ABS(DBLE(Z)) + ABS(DIMAG(Z))
      RETURN
      END
      INTEGER          FUNCTION ILAENV1( ISPEC, NAME, OPTS, N1, N2, N3,
     $                 N4 )
*
...
*  =======
*
*  ILAENV1 returns problem-dependent parameters for the local
*  environment.  See ISPEC for a description of the parameters.
*
*  In this version, the problem-dependent parameters are contained in
*  the integer array IPARMS in the common block CLAENV and the value
*  with index ISPEC is copied to ILAENV1.  This version of ILAENV1 is
...
*               where k is given by ILAENV1(2,...) and m by ILAENV1(5,...)
...
*          < 0:  if ILAENV1 = -k, the k-th argument had an illegal value.
*
*  Further Details
*  ===============
*
*  The following conventions have been used when calling ILAENV1 from the
*  LAPACK routines:
...
*      passed a value of -1.
*  3)  The parameter value returned by ILAENV1 is checked for validity in
*      the calling subroutine.  For example, ILAENV1 is used to retrieve
*      the optimal blocksize for STRTRI as follows:
*
*      NB = ILAENV1( 1, 'STRTRI', UPLO // DIAG, N, -1, -1, -1 )
...
*
         ILAENV1 = IPARMS( ISPEC )
*
      ELSE IF( ISPEC.EQ.6 ) THEN
*
*        Compute SVD crossover point.
*
         ILAENV1 = INT( REAL( MIN( N1, N2 ) )*1.6E0 )
*
...
*
         ILAENV1 = 1
         IF( ILAENV1.EQ.1 ) THEN
            ILAENV1 = IEEECK1( 0, 0.0, 1.0 )
         END IF
*
...
*
*
      INTEGER          FUNCTION IEEECK1( ISPEC, ZERO, ONE )
*
*  -- LAPACK auxiliary routine (version 3.1) --
*     Univ. of Tennessee, Univ. of California Berkeley and NAG Ltd..
*     November 2006
...
*  =======
*
*  IEEECK1 is called from the ILAENV1 to verify that Infinity and
