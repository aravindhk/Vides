...
void LDOS_Lake(double E,complex ***lowdiag,complex ***diag,complex ***updiag,
	       double ***A1,double ***A2,complex **sigmas,
	       complex **sigmad,int n,
	       int Nc,int flagtrans,double *T,double thop,double eta)
{
  ...
  *A2=dmatrix(0,Nc-1,0,n-1);
	...
	for (k=0;k<n;k++)
	  {
	    if (j==k)
	      {
		if (fabs(E)<1e-10)
		  d[i][j][k]=csum(complass(E+1e-4,eta),diag[i][j][k]);
		else
  ...
  G=rgfblock_Lake(lowdiag,d,updiag,n,Nc,1);
  ...
  for (i=0;i<Nc;i++)
	...
	(*A2)[i][j]=temp[j];
      ...
      //The local density of states for the left contact
      //is equal to A=i*(G^r-G^a)-A2
      for (j=0;j<n;j++)
	{
	  (*A1)[i][j]=-2*G[i][0][j][j].i/(2*pi)-(*A2)[i][j];
	}
    }
  ...
  if (flagtrans==1)
    {
      *T=transmission(G[0][1],gamma2,gamma1,n);
    }
