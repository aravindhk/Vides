...
G0=loadtxt("g0.dat");
plot(G0[:,0],G0[:,1]);
print((sum(T),sum(G0[:,1])))
show()

a=[H.E,T]
savetxt("testo",transpose(a));

if (abs(sum(T)-sum(G0[:,1]))<4e2):
    string="PASSED"
else:
    string="NOT PASSED"
...
if (abs(sum(T)-sum(G[:,1]))<500):
    string2="PASSED"
else:
    string2="NOT PASSED"

fp=open("graphene_test","w");
string3="TEST on graphene \n Test on T for null field: %s \n Test on T for not-zero field: %s \n" %(string,string2);
fp.write(string3);
fp.close()
